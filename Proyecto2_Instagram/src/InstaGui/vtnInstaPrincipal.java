...
public class vtnInstaPrincipal extends JFrame implements PostListener {
    ...
    public vtnInstaPrincipal(Usuario usuario) {
        ...
        setTitle("INSTA - @" + usuario.getUsuario()); //cambio de NombreUsuario a Usuario
        setSize(390, 844);
    ...
    private void iniciarConexionChat() {
        new Thread(() -> {
            try {
                ...
                System.out.println("Conectado al servidor de Chat como: " + usuarioActual.getUsuario());

                while (true) {
                    Mensaje msjRecibido = (Mensaje) entradaSocket.readObject();
                    recibirMensajeEnTiempoReal(msjRecibido);
                }
            } catch (Exception e) {
                System.err.println("No se pudo conectar al servidor de chat (¿Está encendido?).");
            }
        }).start();
    }
    ...
    private void recibirMensajeEnTiempoReal(Mensaje msj) {
        ...
        System.out.println("¡Nuevo mensaje de " + msj.getEmisor() + ": " + msj.getContenido());
    }
}
